...
<!-- 댓글 시작 { -->
<section id="bo_vc" style="background-color:#f9f9f9; padding:0px 20px 0px 20px;">
    <h2>답변목록</h2>
    <?php
    $cmt_amt = count($list);
    for ($i=0; $i<$cmt_amt; $i++) {
    ...
    <article id="c_<?php echo $comment_id ?>" <?php if ($cmt_depth) { ?>style="padding-left:<?php echo $cmt_depth ?>px;border-top-color:#e0e0e0;"<?php } ?>>
        <header style="z-index:<?php echo $cmt_sv; ?>">
			...
			<div style="line-height:25px;"><?php echo $list[$i]['name'] ?>님의 답변 <span class="bo_vc_hdinfo"><i class="oi oi-clock" aria-hidden="true"></i> <time datetime="<?php echo date('Y-m-d\TH:i:s+09:00', strtotime($list[$i]['datetime'])) ?>"><?php echo $list[$i]['datetime'] ?></time></span>
    ...
    <?php } ?>
    <?php if ($i == 0) { //답변이 없다면 ?><p id="bo_vc_empty">답변을 준비중 입니다.</p><?php } ?>
